...
export class AdminMoviePageComponent implements OnInit {
  ...
  submit(): void {
    ...
    if (this.isEditing && this.editingId) {
      this.movieService.updateMovie(this.editingId, payload).subscribe(() => {
        this.snackBar.open('Movie updated', 'Close', { duration: 2500 });
        this.loadMovies();
        this.cancelEdit();
      });
    } else {
      this.movieService.createMovie(payload).subscribe(() => {
        this.snackBar.open('Movie created', 'Close', { duration: 2500 });
        this.loadMovies();
        this.movieForm.reset({
          typeId: this.movieTypes[0].id,
          ticketPrice: 0
  ...
  delete(movie: Movie): void {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: 'Delete movie',
        message: `Remove ${movie.name}? This cannot be undone.`,
        confirmLabel: 'Delete'
      }
    });
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.movieService.deleteMovie(movie.id).subscribe(() => {
          this.snackBar.open('Movie deleted', 'Close', { duration: 2500 });
          this.loadMovies();
        });
      }
    });
