describe('Movie Ticketing flow', () => {
  ...
  it('creates a movie, adds it to cart, and checks out', () => {
    ...
    cy.wait('@postCart');
    cy.get('mat-snack-bar-container').should('contain', 'Added to cart');
    ...
    cy.wait('@postMovie');
    cy.get('mat-snack-bar-container').should('contain', 'Movie created');

    cy.visit('/cart');
    cy.wait('@getCart');
    cy.get('button').contains('Complete checkout').click();
    cy.wait('@checkout');
    cy.get('mat-snack-bar-container').should('contain', 'Checkout completed');
  });
});
